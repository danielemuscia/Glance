...
// Two pre-built NSImages: inactive (template, system color) and active (G in labelColor,
// dot in glanceAccent). The drawing handler is called at render time so both images
...
struct GlanceStatusIcon: View {
    ...
    var body: some View {
        Image(nsImage: isMeetingActive ? Self.activeImage : Self.inactiveImage)
            .resizable()
